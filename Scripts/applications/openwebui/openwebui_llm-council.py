"""
LLM webui council tool

SOURCE
======
https://github.com/matheusbuniotto/openwebui-tools
somewhere in there the llm council tool


CHANGES
=======
- all stuff to work with my local setup rather tahn cloud shit
  - poibnt urls etc.
- instances of same model (initially)
...
#DEFAULT_COUNCIL_MODELS = "openai/gpt-oss-20b"
# repaeat same for multi isnatance same shit
DEFAULT_COUNCIL_MODELS = "openai/gpt-oss-20b,openai/gpt-oss-20b,openai/gpt-oss-20b"


class Tools:
    class Valves(BaseModel):
        openwebui_base_url: str = Field(
            default="http://open-webui:8080/api",  # u don't use the default stuff
        ...
        openwebui_api_key: str = Field(
            default="",  # it define in .env AND as valve in webui
        ...
        # todo(evan0:)this shit
        '''
        stage1_temperature: float = Field(
            default=0.8,
            description="sampling temp for stage 1"
        )
        stage2_temperature: float = Field(
            default=0.8,
            description="sampling temp for stage 2"
        )
        stage3_temperature: float = Field(
            default=0.8,
            description="sampling temp for stage 3"
        )
        max_tokens: int = Field(
            default=4096,  # e.g. cap 512-1500 if latency become issue, dep on prompt
            description='max toks used'
        )
        top_p: float = Field(
            default=0.9,
            description='top p samp'
        )
        '''
    ...
    def _resolve_base_url(self) -> str:
        ...
        docker_url = "http://open-webui:8080/api"
    ...
    def _query_model_sync( # DIFF
        ...
        payload = {
            "model": model,
            "messages": messages,
            #"temperature":   # TODO(evan): do the plumbign between each stage_N
        }
